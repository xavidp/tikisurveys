...
writeBin(bin, file.path(getwd(), dataRelDir, my.tracker.file))
```
...
my.df <- readr::read_csv(file.path(getwd(), dataRelDir, my.tracker.file))
#str(my.df)
head(my.df[,1:10])

#class(gsub("<span class='formunit'>#</span>", "", my.df[,2], fixed=T))
# Clean col 2 (self numbered column in Tiki with this annoying span html class)
my.df[,2] <- gsub("<span class='formunit'>#</span>", "", my.df[,2], fixed=T)
...
##grep("S\\u00cd", my.tfd[,"options"], fixed=T)
##grep("S\\u00ed", my.tfd[,"options"], fixed=T)
...
require(sjmisc)
require(sjPlot)
view_df(data.frame(my.df.c), file=file.path(resultsRelDir, paste0("view_df_my.df.c", ".html")),
        showFreq = TRUE, showPerc = TRUE)
