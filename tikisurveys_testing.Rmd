...
my.field.data <- set_val_labels(my.field.data, memisc::labels(my.field.labels))
get_val_labels(my.field.data, include.values = T)
view_df(data.frame(my.field.data))
...
for (ff in 1:length(fields.with.options.fieldId)) {
  ...
  my.field.data.sj <- set_val_labels(my.field.data,
                                               memisc::labels(my.field.data.labels))
  # Set also labels in the format expected by sjmisc with set_val_labels()
