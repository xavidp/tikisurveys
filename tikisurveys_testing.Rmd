...
my.df <- readr::read_csv(file.path(getwd(), my.tracker.file))
#str(my.df)
head(my.df[,1:10])
...
for (ff in 1:length(fields.with.options.fieldId)) {
  ...
  if (sum(as.numeric(as.character(my.field.data)), na.rm=T) > 0) {
    ...
    my.field.data.labels.memisc <- as.item(my.field.data, 
                                        labels=structure(my.op.tmp.number, 
                                                         names=my.op.tmp.label),
                                        missing.values=99,
                                        annotation = c(
...
#     
  ...
  try(view_df(as.data.frame(my.field.data.labels.sjmisc), file=paste0("view_df_ff", ff,
