...
## Test the display of a radio button field (or other types of fields)

```{r Test the display of a radio button field (or other types of fields)}
...
if (check.one.rb.field) {
    ...
    #my.field.colname.test <- "AnoxToxMet. Hidroelectro Tipo -- 549" # As a Column name of my.df
      # "TAC evol 48h -- 73" # Type M - Multiple select field type
      # data: 
      # [1] "2, 3" "1, 3" "2, 3" "3"    "0"    ""     ""     ""     "0"    ""     ""     "1"    ""     ""     ""     "0"  
    my.rb.test.field <- 2
    my.rb.test.field.row <- which(my.tfd$fieldId==my.rb.test.field)
    my.tfd[which(my.tfd$fieldId==my.rb.test.field),"options"]
    my.options.test <- unlist(fromJSON(as.character(my.tfd[which(as.numeric(my.tfd$fieldId) ==
                                                                   my.rb.test.field),"options"])))
    my.op.tmp.test <- unlist(str_split(my.options.test, "="))
    
    my.tfd[my.rb.test.field.row,]
    
    my.field.name.test <- my.tfd[which(my.tfd$fieldId==my.rb.test.field),"name"]
    my.field.id.test <- my.rb.test.field
    ...
    # The which !is.na() below is needed for cases of multiselect tracker field types
    my.labels.df.test <- suppressWarnings(data.frame(as.numeric(my.op.tmp.number.test)[which(!is.na(as.numeric(my.op.tmp.number.test)))],
                               as.character(my.op.tmp.label.test)[which(!is.na(as.character(my.op.tmp.label.test)))],
                               stringsAsFactors=FALSE)) 
    colnames(my.labels.df.test) <- c("value", "label")
    # Pending ToDo: Convert variable into a dummy list of values splitted by comma, containing vectors of values splitted apart  
    ...
    # If there are more labels than levels of values, avoid the error.
     # DISABLED condition with a hardcoded FALSE in the if clause
    if (FALSE && length(table(my.field.labels.test)) != length(table(memisc::labels(my.field.labels.test)))) {
...
## Play with numbers & Tweak numeric, computed and math variables (convert to character)

```{r Play with numbers & Tweak numeric computed and math variables  (convert to character)}
  # Convert to factor also the numeric values
fields.numbers.idx <- grep("^n$", my.tfd$type, fixed=FALSE)
length(fields.numbers.idx)
# 42
# Add text fields that carry numeric values. They were defined as "text" (type "t") in the tiki definition fields, but readr got them as numeric, since they are mostly carrying numeric values.. 
# Example: field NHC
#grep("NHC", my.tfd[,3])
# [1] 119
#my.tfd[119,]
# N  fieldId trackerId name         permName  options type (...)
#137       7         1  NHC h0_PPaciente_NHC    (...)    t (...)
# Therefore we add that case also to the numbic variables
fields.numbers.idx <- c(fields.numbers.idx, grep("NHC", my.tfd[,3]))
length(fields.numbers.idx)
# 43
#grep("Etnia", my.tfd[,3])
#my.tfd[114,]

fields.math.idx <- grep("^math$|^C$", my.tfd$type, fixed=FALSE)
length(fields.math.idx)
my.tfd[fields.math.idx,]
# Get the fieldId's corresponding to number type of Field
fields.numbers.fieldId <- my.tfd[fields.numbers.idx,"fieldId"]
fields.numbers.fname <- my.tfd[fields.numbers.idx,"name"]
fields.numbers.fname.full <- paste0(fields.numbers.fname , " -- ", fields.numbers.fieldId) # As a Column name of my.df
fields.math.fieldId <- my.tfd[fields.math.idx,"fieldId"]
fields.math.fname <- my.tfd[fields.math.idx,"name"]
fields.math.fname.full <- paste0(fields.math.fname , " -- ", fields.math.fieldId) # As a Column name of my.df

play.with.numbers <- F # F # T
if (play.with.numbers) {
    # Test for the missing info in Dates
    my.num.test.field <- 7
    my.num.test.field.row <- which(my.tfd$fieldId==my.num.test.field)
    # [1] 118
    my.tfd[which(my.tfd$fieldId==my.num.test.field),"options"]
    #[1] "{\"datetime\":\"d\",\"startyear\":\"1900\",\"endyear\":\"2016\",\"blankdate\":\"blank\"}"
    #table(as.numeric(my.tfd$fieldId)==my.num.test.field)
    #class(my.tfd$fieldId)
    my.options.test <- unlist(fromJSON(as.character(my.tfd[which(as.numeric(my.tfd$fieldId) ==
                                                                   my.num.test.field),"options"])))
    my.op.tmp.test <- unlist(str_split(my.options.test, "="))
    
    #head(my.df[,1:10])
    #length(colnames(my.df))
    #[1] 274
    my.tfd[my.num.test.field.row,]
    
    my.field.name.test <- my.tfd[which(my.tfd$fieldId==my.num.test.field),"name"]
    my.field.id.test <- my.num.test.field
    my.field.colname.test <- paste0(my.field.name.test , " -- ", my.field.id.test) # As a Column name of my.df
    my.field.data.test <- as.data.frame(my.df.c[,my.field.colname.test])[,1]
...
# Change class from numeric to character so that numbers are also shown in the view_df html produced
my.df.c.strings[,fields.numbers.fname.full] <- lapply(my.df.c[,fields.numbers.fname.full] , as.character)
my.df.c.strings[,fields.math.fname.full] <- lapply(my.df.c[,fields.math.fname.full] , as.character)
...
#    merged.df <- merge(df1, df2, by = intersect(names(df1), names(df2)), all.x=T)
    ...
    # If there are more labels than levels of values, avoid the error.
    # DISABLED condition with a hardcoded FALSE in the if clause
    if (FALSE && length(table(my.field.data)) != length(table(memisc::labels(my.field.data.labels.memisc)))) {
...
#     
    ...
    my.df.c.strings[,my.df.c.col.idx] <- my.field.data.labels.sjmisc
  ...
  if (any(grepl("Error", try(view_df(as.data.frame(my.field.data.labels.sjmisc), 
                                   file=file.path(logsRelDir, paste0("view_df_ff", sprintf("%03d", ff),
                                                  "_fieldId", sprintf("%03d", my.field.id),
                                                  "_col", sprintf("%03d", my.df.c.col.idx),
                                                  "_", my.field.name,
    ...
    CSS = NULL, useViewer = TRUE, no.output = FALSE, remove.spaces = TRUE) 
{
    ...
    encoding <- my.c.encoding# get.encoding(encoding, x)
    if (!is.data.frame(x)) 
    ...
    for (ccnt in 1:colcnt) {
        ...
        if (showFreq) {
            page.content <- paste0(page.content, sprintf("    <td class=\"tdata%s\">%s</td>\n", 
                arcstring, frq.value2(index, x, df.val)))
        }
        if (showPerc) {
            page.content <- paste0(page.content, sprintf("    <td class=\"tdata%s\">%s</td>\n", 
        ...
        if (showWtdFreq && !is.null(weightBy)) {
            page.content <- paste0(page.content, sprintf("    <td class=\"tdata%s\">%s</td>\n", 
                arcstring, frq.value2(index, x, df.val, weightBy)))
        }
...
frq.value2 <- function (index, x, df.val, weights = NULL) 
{
    valstring <- ""
    if (index <= ncol(x) && !is.null(df.val[[index]])) {
        ...
        ftab <- create.frq.df2(variab, 20)$mydat$frq
...
create.frq.df2 <- function (x, breakLabelsAt = Inf, order.frq = "none", round.prz = 2, 
    na.rm = FALSE, weightBy = NULL) 
{
    if (length(stats::na.omit(x)) == 0) {
        mydat <- data.frame(val = NA, label = NA, frq = NA, raw.prc = NA, 
            valid.prc = NA, cum.perc = NA, upper.ci = NA, lower.ci = NA)
        return(invisible(structure(list(mydat = mydat))))
    }
    labels <- sjmisc::get_labels(x, attr.only = T, include.values = "n", 
        include.non.labelled = T)
    if (!is.null(weightBy)) 
        x <- sjmisc::weight(x, weightBy)
    if (!is.null(labels)) {
        dat <- data.frame(n = names(labels), v = as.character(labels), 
            stringsAsFactors = FALSE)
        ...
        else dat$val <- as.numeric(dat$val)
    ...
    if (!is.null(order.frq) && (order.frq == "asc" || order.frq == 
        "desc")) {
        ord <- order(mydat$frq[1:valid.vals], decreasing = (order.frq == 
            "desc"))
        mydat <- mydat[c(ord, valid.vals + 1), ]
        labels <- labels[ord]
    }
    mydat$raw.prc <- mydat$frq/sum(mydat$frq)
    mydat$valid.prc <- c(mydat$frq[1:valid.vals]/length(stats::na.omit(x)), 
        NA)
    mydat$cum.prc <- c(cumsum(mydat$valid.prc[1:valid.vals]), 
        NA)
    mydat$raw.prc <- 100 * round(mydat$raw.prc, round.prz + 2)
    mydat$cum.prc <- 100 * round(mydat$cum.prc, round.prz + 2)
    mydat$valid.prc <- 100 * round(mydat$valid.prc, round.prz + 
        2)
    if (na.rm) {
    ...
    invisible(structure(list(mydat = mydat, labels = labels, 
        minval = min(sjmisc::to_value(mydat$val, keep.labels = FALSE), 
            na.rm = TRUE))))
}
...
for (cc in 1:length(colnames(my.df.c))) {
    view_df(data.frame(my.df.c.strings[,colnames(my.df.c)[cc]]), 
            file=file.path(logsRelDir, paste0(my.base.filename,"_view_df_col", 
                                                 sprintf("%03d", cc),".html")),
            showFreq = TRUE, showPerc = TRUE, encoding = "UTF-8", showType=TRUE)
}
 
view_df2(data.frame(my.df.c.strings), 
        file=file.path(resultsRelDir, paste0(my.base.filename,"_view_df.html")),
        showFreq = T, showPerc = T, encoding = "UTF-8", showType=F)
# Produce the equivalent html but to be embedded in the knitted html produced, not as extra html file on disk
# view_df(data.frame(my.df.c.strings), 
#         showFreq = TRUE, showPerc = TRUE, encoding = "UTF-8", showType=TRUE, 
#         no.output=TRUE)$knitr

sjt.df(as.data.frame(my.df.c.numbers),
       file=file.path(resultsRelDir, paste0(my.base.filename,"_sjt.df.description.html")),
       showType=FALSE, describe=TRUE)
sjt.df(as.data.frame(my.df.c.numbers),
       file=file.path(resultsRelDir, paste0(my.base.filename,"_sjt.df.values.html")),
       showType=FALSE, describe=FALSE)
